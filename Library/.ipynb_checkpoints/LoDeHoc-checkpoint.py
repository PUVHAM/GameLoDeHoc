...
def nhap_tien_cuoc(username,so_lo):
    ...
    while True:
        ...
        if tien_cuoc.isdigit() and 0 < int(tien_cuoc) * so_lo <= tong_tien:
...
def het_tien(username, so_tien_nap):
    tong_tien = int(lay_thong_tin_tai_khoan(username)[2])
    if tong_tien == 0:
        while True:
            check = int(input("Bạn đã hết tiền, có muốn vay tiền để chơi tiếp? (Nhập '0': không, nhập '1': có): "))
            if check == 0:
                ...
                so_tien_nap = int(input("Nhập số tiền cần vay: "))
                nap_tien(username, so_tien_nap)
                ghi_no(username, so_tien_nap)
